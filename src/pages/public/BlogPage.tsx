  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 20px 70px' }}>
        <p style={{ color: 'var(--gold)', letterSpacing: '.25em', fontSize: '.78rem', margin: 0 }}>TODO SOBRE LAS BOAT PARTIES</p>
        <h1 className="bebas" style={{ fontSize: 'clamp(2.6rem, 8vw, 4.5rem)', margin: '10px 0 8px', lineHeight: .95 }}>Blog & Noticias</h1>
        <p className="text-muted-c" style={{ maxWidth: 520, margin: '0 0 30px' }}>
          Guías, consejos, lineup y lo mejor de Gran Canaria desde el mar.
        </p>
        ...
        {posts === null ? (
          <p className="text-muted-c">Cargando…</p>
        ) : visible.length === 0 ? (
          <div className="event-card" style={{ padding: 32, cursor: 'default', maxWidth: 520 }}>
            <p className="bebas" style={{ fontSize: '1.3rem', margin: '0 0 6px' }}>No hay artículos en esta categoría todavía</p>
            <p className="text-muted-c" style={{ margin: 0, fontSize: '.9rem' }}>
              Estamos escribiendo. Mientras tanto, la fiesta sigue en el barco.
            </p>
          </div>
        ) : (
              ...
              <Link key={p.id} to={`/blog/${p.slug}`} className="event-card" style={{ textDecoration: 'none', color: 'inherit' }}>
                ...
                <div style={{ padding: '16px 18px 20px' }}>
                  <span style={{ color: 'var(--gold)', fontSize: '.7rem', letterSpacing: '.2em' }}>
                    {CATEGORIES.find(c => c.key === p.category)?.label.toUpperCase()}
                  </span>
